...
BASE_URL = "https://vos.olimpiada.ru/team/year/{year}/staff"

START_MARKER_A = "Состав команды Москвы"
...
def parse_staff(year: int) -> list[list[str]]:
    """
    Parse https://vos.olimpiada.ru/team/year/<year>/staff and return rows:
    [surname, name, patronymic, school, grade, subject]
    """
    ...
    for line in lines[start_idx:]:
        ...
        if subject:
            # Subject lines mark the beginning/continuation of data blocks.
            current_subject = subject
            current_grade = None
            continue

        participant_match = PARTICIPANT_RE.match(line)
        if participant_match and current_subject and current_grade:
            ...
            rows.append([surname, name, patronymic, school, current_grade, current_subject])
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Parse participants from vos.olimpiada.ru/team/year/<year>/staff"
    )
    parser.add_argument("year", type=int, help="Year in URL, for example 2026")
    args = parser.parse_args()

    result = parse_staff(args.year)
    print(result)
