        ...
        for task in tqdm(tasks):
            ...
            if r:
                ...
                for data in result:
                    ...
                    prefixes = ["Первая", "Вторая", "Третья"]
                    for number, sol in enumerate(resources):
                        text = sol.get("text", 0)
                        if not text:
                            continue
                        text = text.strip("`")
                        ...
                        text = text.strip()
                        if len(resources) > 1:
                            solve_string += f"# {prefixes[number]} часть решения"
                        solve_string += f"\n\n{text}\n\n"
                if not solve_string:
                    continue
                ...
                if num == 6 and "вперед" in solve_string:
                    ex = "kum"
                elif "/*" in solve_string and "*/" in solve_string:
                    ex = "c"
                else:
                    ex = "py"
                with open(
